...
- **[PersonalS3](https://personals3.tech)** — Self-hosted S3-compatible object storage with built-in media transcoding. Sign in, drag-drop a video → it transcodes to HLS in the background, exposes signed share URLs, and streams globally via Cloudflare Tunnel. Ships a cross-platform `ps3` CLI, S3 SDK compatibility (boto3 / aws-cli / rclone), self-serve onboarding with 2FA + trusted devices, and [user docs](https://developers.personals3.tech) on Cloudflare Pages
